 ...
 * 备       注：无
******************************************************************/
unsigned char I2C_WaitAck1(void)
{
...
char MPU6050_WriteReg(uint8_t addr,uint8_t regaddr,uint8_t num,uint8_t *regdata)
{
    ...
    IIC_Start();
    Send_Byte1((addr<<1)|0);
    if( I2C_WaitAck1() == 1 ) {IIC_Stop();return 1;}
    Send_Byte1(regaddr);
    if( I2C_WaitAck1() == 1 ) {IIC_Stop();return 2;}

    for(i=0;i<num;i++)
    {
        Send_Byte1(regdata[i]);
        if( I2C_WaitAck1() == 1 ) {IIC_Stop();return (3+i);}
    }
...
char MPU6050_ReadData(uint8_t addr, uint8_t regaddr,uint8_t num,uint8_t* Read)
{
        ...
        IIC_Start();
        Send_Byte1((addr<<1)|1);
        if( I2C_WaitAck1() == 1 ) {IIC_Stop();return 3;}
